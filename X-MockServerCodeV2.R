...
parl.select <- data.frame(parliament = c("UK-HouseOfCommons", "DE-Bundestag"),
                          dev.path = c("./PLS-words/Data/hc_tok.rds", "./PLS-words/Data/bt_corp.feather"), # relative path from project directory
...
out.tok <- read_feather(parl.select$dev.path[parl.select$parliament == user.parliament])

out.data <- out.tok %>% 
  mutate(hits = str_count(text, user.regex),
         user.words.pres = as.numeric(hits > 0)) # are user words present in speech?
